...
export function portraitImage(src: string | undefined, variant: PortraitVariant) {
  if (!src) return "";
  return `/optimized/portraits/${filenameWithoutExtension(src)}-${variant}-${portraitSizes[variant]}.jpg`;
}

export function flyerImage(src: string, variant: FlyerVariant) {
  return `/optimized/flyers/${filenameWithoutExtension(src)}-${variant}-${flyerSizes[variant]}.jpg`;
}
