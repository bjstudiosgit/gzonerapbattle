...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${isOpen || !isHome ? 'bg-zinc-950' : 'bg-transparent'}`}>
          ...
          <div className="flex items-center shrink-0">
            <Link to="/" onClick={() => setIsOpen(false)} className="flex items-center gap-2 group/logo">
              ...
              <div className="flex flex-col leading-none">
                <span className="font-black text-xl tracking-[0.2em] uppercase leading-none text-white group-hover:text-brand transition-colors">THE <span className="text-brand group-hover:text-white">GZONE</span></span>
                <span className="text-[9px] text-zinc-500 uppercase font-black tracking-[0.25em] mt-1.5 opacity-60">In Association with <br /> Peacock Gymnasium</span>
              </div>
            </Link>
          </div>
            ...
            <div className="ml-8 flex items-center space-x-1 lg:space-x-2">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`relative px-4 py-2 rounded-full text-[12px] font-black uppercase tracking-[0.2em] transition-all duration-300 flex items-center gap-2 group/nav
            ...
            <div className="px-4 py-8 space-y-2">
              {navLinks.map((link) => (
                <Link
                  ...
                  to={link.href}
                  onClick={(e) => handleLinkClick(e, link.href)}
                  className={`flex items-center gap-6 px-6 py-6 rounded-[1.5rem] text-lg font-display uppercase tracking-widest transition-all
