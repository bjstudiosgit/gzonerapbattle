...
export function calculateRankings(
  ...
  const releasedBattles = singlesBattles.filter((b) => !b.isUnreleased);
  ...
  for (const battle of releasedBattles) {
  ...
  for (const battle of releasedBattles) {
    ...
    if (winnerRank !== undefined && loserRank !== undefined) {
      if (winnerRank > loserRank) {
        ...
        if (battle.bonusPoints !== undefined) {
          bonus = battle.bonusPoints;
        } else if (positionDiff >= 1 && positionDiff <= 3) bonus = 1;
    ...
    if (battle.performancePoints) {
      for (const [mcId, points] of Object.entries(battle.performancePoints)) {
        const mcStats = statsMap.get(mcId);
        if (mcStats) {
          mcStats.bonusScore += points;
        }
      }
    }
  }
