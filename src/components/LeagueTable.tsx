      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {showTitle && (
          <div className="text-center mb-16 md:mb-24">
            ...
            <p className="text-zinc-500 text-[10px] md:text-sm font-black tracking-[0.4em] opacity-80 max-w-2xl mx-auto">
            ...
            <table className="w-full text-left border-collapse min-w-[850px] md:min-w-0">
              <thead>
                <tr className="bg-zinc-950/80 border-b border-white/10">
                  ...
                  <th className="px-6 py-6 md:px-10 md:py-10 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500">MC</th>
                  <th className="px-4 py-6 md:px-6 md:py-10 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 text-center whitespace-nowrap">Wanted Level</th>
                  ...
                  <th className="px-6 py-6 md:px-10 md:py-10 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 text-center">Points</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {rankings.map((mc, index) => {
                  const isTop3 = index < 3 && !mc.isDsq;
                  const wantedStars = Math.max(getRankStars(mc.rank), mc.wins > 0 ? 1 : 0);
                  return (
                    <motion.tr
                      key={mc.id}
                      initial={{ opacity: 0, y: 20 }}
                    ...
                    >
                        ...
                        <Link to={`/mc/${mc.slug}`} className="flex items-center gap-4 group/name relative">
                          ...
                          <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              {mc.isDsq ? (
                                <span className="text-[7px] md:text-[9px] font-black uppercase tracking-[0.2em] px-2 py-0.5 bg-zinc-900 border border-zinc-700/50 rounded-md text-zinc-300">WASTED</span>
                              ) : mc.nickname ? (
                                <span className="text-[7px] md:text-[9px] font-black uppercase tracking-[0.2em] text-zinc-600">
                                  {mc.nickname}
                                </span>
                              ) : null}
                            </div>
                      ...
                      <td className="px-4 py-8 md:px-6 md:py-10">
                        {mc.isDsq ? (
                          <span className="block text-center text-zinc-700">-</span>
                        ) : (
                          <div
                            className="mx-auto flex w-fit items-center gap-1 rounded-full border border-yellow-500/15 bg-yellow-500/[0.04] px-3 py-2 shadow-[inset_0_0_18px_rgba(234,179,8,0.03)]"
                            aria-label={`${wantedStars} out of 5 Wanted stars`}
                          >
                            {Array.from({ length: 5 }, (_, starIndex) => {
                              const isEarned = starIndex < wantedStars;
                              return (
                                <Star
                                  key={starIndex}
                                  size={14}
                                  ...
                                  className={
                                    isEarned
                                      ? "fill-yellow-500 text-yellow-500 drop-shadow-[0_0_6px_rgba(234,179,8,0.45)]"
                      ...
                      <td className={`px-6 py-8 md:px-10 md:py-10 text-center font-display text-2xl md:text-3xl ${isTop3 ? 'text-brand drop-shadow-[0_0_10px_rgba(242,125,38,0.3)]' : 'text-zinc-500'}`}>
                        {mc.battles === 0 ? "-" : mc.points.toLocaleString()}
                      </td>
                    </motion.tr>
                  );
                })}
          ...
          <div className="p-1 border-t border-white/10 bg-zinc-950/80">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 px-10 py-10">
              <div className="hidden lg:flex items-center gap-4 group cursor-help">
                ...
                <p className="text-[9px] md:text-[10px] text-zinc-500 font-black tracking-[0.2em] leading-relaxed max-w-sm">
                  League data is updated in real-time following each official clash at Peacock Gymnasium.
                </p>
              </div>
