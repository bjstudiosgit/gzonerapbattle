...
export default function MCCarousel({ items, autoAdvanceMs }: CarouselProps) {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label="Previous lineup member"
      >
        <ChevronLeft size={24} />
      </button>
      <button
          ...
          return (
            <button
              ...
              className={`h-2 rounded-full transition-all ${
                index === active ? "w-8 bg-brand" : "w-2 bg-white/20 hover:bg-white/40"
              }`}
            />
          );
        })}
  ...
  if (!isCenter) {
    return (
      <div className="relative w-[240px] sm:w-[320px] aspect-[3/4] rounded-3xl overflow-hidden border border-white/10 bg-zinc-950 pointer-events-none">
        <img src={portraitImage(mc.image, "card")} alt={mc.name} className="w-full h-full object-cover" />
          ...
          {stars > 0 && (
            <div className="flex items-center gap-0.5 mb-2">
              {[...Array(stars)].map((_, index) => (
                <Star key={index} size={12} className="text-yellow-500 fill-yellow-500" />
              ))}
            </div>
          )}
    ...
    >
      ...
      <div className="relative w-full h-full rounded-[2rem] overflow-hidden border border-brand/30 bg-zinc-950 shadow-[0_0_80px_rgba(242,125,38,0.25)]">
        <img
          ...
          alt={item.name}
          loading="lazy"
          className={`w-full h-full object-cover object-top grayscale group-hover:grayscale-0 transition duration-700 ${
            isMystery ? "blur-xl opacity-40" : ""
          }`}
          onError={(event) => {
            const target = event.target as HTMLImageElement;
            if (!target.src.includes("picsum")) {
        ...
        <div className="absolute bottom-0 left-0 right-0 z-30 p-5 sm:p-7">
          ...
          <p className="text-zinc-300 text-xs sm:text-sm leading-relaxed mb-5 line-clamp-3">{bioPreview}</p>
        </div>
      </div>
    </motion.div>
  );
