...
export default function MCProfile() {
  ...
  const badgeLines = mc.name.split(/\s+/).filter(Boolean).slice(0, 2);
  const badgeMaxLineLength = badgeLines.reduce((max, line) => Math.max(max, line.length), 0);
  const badgeTextSizeClass =
    badgeLines.length > 1
      ? badgeMaxLineLength <= 5
        ? "text-2xl"
        : "text-xl"
      : badgeMaxLineLength <= 4
        ? "text-4xl"
        : "text-3xl";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          >
            ...
            <div
              className="absolute -bottom-6 -right-6 w-32 h-32 bg-brand rounded-full flex flex-col items-center justify-center text-black -rotate-12 z-20 shadow-xl border border-black/10"
              aria-label={`${mc.name} badge`}
            >
              <div className="text-[10px] font-black uppercase tracking-[0.35em] opacity-80">MC</div>
              <div
                className={`font-display uppercase leading-none text-center px-3 ${badgeTextSizeClass}`}
                title={mc.name}
              >
