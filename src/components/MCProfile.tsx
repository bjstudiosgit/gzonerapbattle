...
export default function MCProfile() {
  ...
  const mcProps = mcBattles
    ...
    .filter((prop, index, props) => props.findIndex(item => item.name === prop.name) === index);
  const form = mcBattles
    .filter((battle) => Boolean(battle.winner))
    .map((battle) => battle.winner === mc.id ? "W" : "L");
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          ...
          >
            ...
            <div className={`relative aspect-[4/5] md:aspect-[3/4] max-h-[430px] md:max-h-none mx-auto rounded-3xl overflow-hidden border border-white/10 shadow-2xl shadow-brand/20 z-10 ${isWasted ? 'grayscale' : ''}`}>
              <img 
              ...
              {rank > 0 && rank <= 3 && !isWasted && (
                <div className="absolute top-1/2 left-1/2 z-30 w-max -translate-x-1/2 -translate-y-1/2 scale-75 pointer-events-none md:scale-90">
                  <div className="transform -rotate-12 border-4 border-red-600 bg-black/40 px-5 py-2 backdrop-blur-sm shadow-[0_0_50px_rgba(220,38,38,0.3)]">
                    <span className="whitespace-nowrap font-display text-5xl uppercase tracking-[-0.06em] text-red-600 drop-shadow-[0_0_15px_rgba(220,38,38,0.5)] md:text-6xl">
                      MOST WANTED
                    </span>
                  </div>
                </div>
          ...
          >
            <div className="mb-8">
              <div className="flex flex-col gap-4 mb-4">
                ...
                <h1 className="text-5xl md:text-7xl font-display uppercase leading-tight flex items-center gap-3 md:gap-4">
                  {rank > 0 && <span className="text-zinc-600 text-3xl md:text-5xl">#{rank}</span>}
                  {mc.name}
                  {mc.id === 'ajna' && <Crown className="text-brand" size={48} />}
                </h1>
                {form.length > 0 && (
                  <div className="flex items-center gap-3" aria-label={`${mc.name} form: ${form.join("")}`}>
                    <span className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em]">Form</span>
                    <div className="flex items-center gap-1.5">
                      {form.map((result, index) => (
                        <span
                          key={`${result}-${index}`}
                          className={`flex h-7 w-7 items-center justify-center rounded-md border text-xs font-black ${
                            result === "W"
            ...
            {mcProps.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-display uppercase mb-6 text-white border-l-4 border-brand pl-4">Props Used</h2>
                <div className="grid gap-4 sm:grid-cols-2">
                  {mcProps.map(prop => (
                    <div key={prop.name} className="flex items-center gap-4 bg-zinc-900/50 p-4 rounded-2xl border border-white/5">
                      <span className="text-2xl" aria-hidden="true">{prop.icon}</span>
