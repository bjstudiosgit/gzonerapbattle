    ...
    >
      <Link to={`/mc/${mc.slug}`} className="block w-full h-full" aria-label={`View ${mc.name} profile`}>
        ...
        {isLockedUp && (
          <>
            ...
            <div className="absolute inset-0 flex items-center justify-center z-30">
              <span className="bg-black/80 text-white px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.3em] border border-white/10">COOLING OFF</span>
            </div>
          </>
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 p-4 flex flex-col justify-end">
          <div className="mb-1">
            <h4 className="text-2xl font-display uppercase leading-none group-hover:text-brand transition-colors truncate">{mc.name}</h4>
