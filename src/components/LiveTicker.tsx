...
export default function LiveTicker() {
  ...
  return (
    <div className="w-full bg-brand text-black py-2 overflow-hidden whitespace-nowrap border-y border-black/20 relative z-50 shadow-[0_0_30px_rgba(242,125,38,0.3)]">
      <div
        className="flex items-center px-4 md:w-max md:gap-12 md:animate-marquee"
        style={{ animationDuration: "60s" }}
      >
        {tickerItems.map((item, idx) => (
          <div key={idx} className={`flex items-center gap-3 shrink-0 ${idx > 0 ? "hidden md:flex" : "w-full overflow-hidden text-ellipsis whitespace-nowrap"}`}>
