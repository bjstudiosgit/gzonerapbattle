      ...
      <AnimatePresence>
        {!videoEnded ? (
          ...
          <motion.div
            ...
            className="relative min-h-[calc(100svh-2rem)] md:min-h-screen flex items-center pt-24 md:pt-32 pb-10 md:pb-20 z-30"
          >
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 w-full">
              <div className="grid lg:grid-cols-[1.3fr_0.7fr] gap-0 items-center relative">
                <motion.div
                  ...
                  className="max-w-4xl relative z-10 lg:pr-8"
                >
                  <div className="mb-5 md:mb-10">
                    <div className="mb-3 md:mb-5 flex items-center justify-center gap-2 md:gap-3 text-yellow-500 md:justify-start">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className="fill-yellow-500 drop-shadow-[0_0_12px_rgba(234,179,8,0.4)] animate-pulse"
                          size={20}
                          style={{ animationDelay: `${i * 120}ms` }}
                        />
                      ))}
                    </div>
                    <h1 className="text-xl md:text-5xl font-display leading-[1.05] uppercase tracking-tight text-center md:text-left">
                      This is the Gzone "Most Wanted" Baby <span className="text-brand text-3xl sm:text-4xl md:text-7xl lg:text-[5rem] xl:text-[5.5rem] block mt-2">Where we dont Play!</span>
                    </h1>
                  </div>
                  
                  <p className="text-zinc-200 text-sm md:text-base font-medium tracking-normal max-w-2xl mx-auto md:mx-0 mb-5 md:mt-8 md:mb-10 leading-relaxed opacity-80 text-center md:text-left">
              Reputations get built or buried. Bars land or they don&apos;t. No hand-holding, no second chances, just moments that make you or break you.
                  </p>
                  
                  {/* Buttons removed as per user request */}
                  
                  <div className="mt-5 pt-4 md:mt-16 md:pt-8 border-t border-white/10 grid grid-cols-5 md:flex items-start justify-center md:justify-start gap-2 md:gap-8 xl:gap-10">
                    <div className="group text-center md:text-left">
                      <div className="text-xl md:text-4xl font-display text-white group-hover:text-brand transition-colors">13.2K</div>
                    ...
                    <div className="w-px h-10 bg-white/10 hidden md:block" />
                    <div className="group text-center md:text-left">
                      <div className="text-xl md:text-4xl font-display text-white group-hover:text-brand transition-colors">{releasedEpisodeCount}</div>
                      <div className="text-zinc-500 text-[8px] md:text-[10px] uppercase tracking-[0.2em] font-bold mt-1">Episodes</div>
                    </div>
                  </div>
                </motion.div>
