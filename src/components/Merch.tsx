  ...
  return (
    <section id="merch" className="py-12 md:py-24 scroll-mt-24 bg-black relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20">
        ...
        <div className="mt-12 p-6 md:p-12 bg-zinc-900 rounded-3xl border border-white/5 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="max-w-md text-center md:text-left">
            <h4 className="text-xl md:text-3xl font-display italic uppercase mb-2">Sign up to the Gzone mailing list</h4>
            <p className="text-zinc-200 text-sm md:text-base">Get early access to limited drops and exclusive event merch.</p>
          </div>
          <div className="flex w-full md:w-auto gap-2">
            <input 
              type="email" 
              placeholder="Enter your email" 
              className="bg-zinc-950 border border-white/10 rounded-xl px-4 py-3 flex-1 md:w-64 focus:outline-none focus:border-brand transition-colors text-sm"
            />
            <button 
              aria-label="Join the G Zone mailing list"
              className="bg-brand text-black px-6 py-3 rounded-xl font-bold text-sm"
            >
              Join
            </button>
          </div>
