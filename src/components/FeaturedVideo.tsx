...
export default function FeaturedVideo() {
  ...
  const featuredHeadline = featuredBattle ? `Latest Battle ${featuredTitle} out now!` : "Latest Battle out now!";
          ...
          <p className="text-zinc-200 text-sm md:text-base font-medium tracking-normal max-w-2xl mx-auto">
            {featuredBattle ? (
              featuredBattle.episode ? (
                <>
                  <span className="font-bold text-white">Episode {featuredBattle.episode}</span> - {featuredBattle.title} is now live in the Gzone arena.
                </>
              ) : (
