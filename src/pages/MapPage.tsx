...
const MapPage: React.FC = () => {
  ...
  const getMarkerPosition = (coords: [number, number]) => {
    ...
    // UK bounds for the gzonemap.png silhouette to match the visual reference
    const minLon = -10.2;
    const maxLon = 2.8;
    const minLat = 49.0;
    const maxLat = 61.2;
  ...
  const hasCoolingOffMC = (location: string) => {
    return mcsByLocation[location].mcs.some(mc => mc.tags?.includes('Wasted'));
  };
    ...
    <div className="min-h-screen pt-32 pb-12 relative overflow-hidden">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-12 md:mb-16">
          <h2 className="text-5xl md:text-9xl font-display uppercase leading-[0.8] mb-6">
            The <span className="text-brand">Gzone</span> Network
          </h2>
          <p className="text-zinc-400 max-w-2xl mx-auto tracking-widest text-[10px] md:text-xs font-black opacity-60">
              ...
              {selectedLocation ? (
                ...
                >
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      <h2 className="text-3xl md:text-5xl font-display uppercase text-white tracking-tight leading-none mb-2">{selectedLocation}</h2>
                      <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-brand animate-pulse" />
                        <p className="text-zinc-500 text-[10px] font-black tracking-widest">{selectedMCs.length} Roster Membership</p>
                      </div>
                    </div>
                    <button 
                        ...
                        >
                          <div className="flex items-center gap-5">
                            ...
                            <div className="flex-1 min-w-0">
                              <h3 className="text-white font-display text-2xl uppercase truncate group-hover:text-brand transition-colors">{mc.name}</h3>
                              <p className="text-zinc-500 text-[10px] font-black tracking-widest truncate mt-0.5 opacity-60">{mc.style}</p>
                            </div>
                ...
                >
                  ...
                  <h3 className="text-2xl font-display uppercase text-zinc-400 mb-3 tracking-widest">Select Node</h3>
                  <p className="text-zinc-600 text-xs font-black tracking-[0.2em] max-w-[240px] leading-relaxed opacity-60">
