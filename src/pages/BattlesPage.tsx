      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 md:mb-16 gap-8 text-center md:text-left">
          ...
          <div className="flex flex-col sm:flex-row gap-4 md:gap-8 w-full md:w-auto">
            <div className="bg-zinc-900/30 border border-white/5 p-4 md:p-8 rounded-3xl flex-1 md:min-w-[180px]">
              ...
              <div className="text-3xl md:text-4xl font-display italic text-white mb-1">{totalViewsStr}</div>
              <div className="flex items-center justify-center md:justify-between mt-1">
                <div className="text-[9px] md:text-[10px] text-zinc-500 uppercase tracking-widest">Last updated: {lastUpdated}</div>
              </div>
            </div>
            <div className="bg-zinc-900/30 border border-white/5 p-4 md:p-8 rounded-3xl flex-1 md:min-w-[180px]">
              <div className="flex items-center justify-center md:justify-start gap-2 text-zinc-400 mb-2">
              ...
              <div className="text-3xl md:text-4xl font-display italic text-white">{battles.length}</div>
            </div>
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      <td className="px-1 py-3 md:px-6 md:py-6">
                        <Link to={`/battle/${battle.slug}`} className="font-mono text-brand text-[9px] md:text-sm hover:underline" aria-label={`Episode 1x${String(index + 1).padStart(2, '0')} details`}>1x{String(index + 1).padStart(2, '0')}</Link>
                      </td>
                      <td className="px-1 py-3 md:px-6 md:py-6">
                      ...
                      <td className="px-1 py-3 md:px-6 md:py-6 text-right md:text-left">
                        <span className={`inline-flex items-center px-1.5 py-0.5 md:px-3 md:py-1 rounded-full text-[6px] md:text-[9px] font-bold uppercase tracking-widest ${
                          battle.videoUrl 
