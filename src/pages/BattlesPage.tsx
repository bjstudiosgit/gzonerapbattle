...
export default function BattlesPage() {
  ...
  const inProductionCount = orderedBattles.filter((battle) => !battle.videoUrl && !battle.isUnreleased).length;
  const outstandingCount = orderedBattles.filter((battle) => battle.isUnreleased).length;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-center lg:items-end justify-between mb-16 md:mb-24 gap-12 text-center lg:text-left">
          ...
          <div className="flex flex-col sm:flex-row gap-4 md:gap-6 w-full md:w-auto">
            ...
            <div className="bg-zinc-900/60 backdrop-blur-xl border border-white/10 p-6 md:p-8 rounded-[2rem] flex-1 md:min-w-[260px] shadow-2xl group hover:border-brand/30 transition-colors">
              ...
              <div className="grid grid-cols-3 gap-4 text-center md:text-left">
                <div>
                  <div className="text-3xl md:text-4xl font-display text-emerald-400 group-hover:text-emerald-300 transition-colors">{liveNowCount}</div>
                  <div className="text-[9px] text-zinc-500 uppercase tracking-widest font-black">Live Now</div>
                </div>
                <div>
                  <div className="text-3xl md:text-4xl font-display text-brand group-hover:text-white transition-colors">{inProductionCount}</div>
                  <div className="text-[9px] text-zinc-500 uppercase tracking-widest font-black">Production</div>
                </div>
                <div>
                  <div className="text-3xl md:text-4xl font-display text-zinc-300 group-hover:text-white transition-colors">{outstandingCount}</div>
                            ...
                            <span className="flex items-center gap-1.5">
                              ...
                              {battle.isUnreleased || shouldShowComingSoon ? (
                                <Link 
                                  ...
                                  className="text-brand animate-pulse underline decoration-brand/30 underline-offset-4"
                                >
                                  {scheduleText}
                                </Link>
