...
export default function BattlesPage() {
  ...
  return (
    <div className="min-h-screen pt-24 md:pt-32 lg:pt-44 pb-16 lg:pb-24 relative overflow-hidden">
      {/* Dynamic Backgrounds */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-center lg:items-end justify-between mb-10 lg:mb-24 gap-8 lg:gap-12 text-center lg:text-left">
          <div className="flex-1 w-full">
            <div className="flex flex-col items-center lg:items-start gap-3 md:gap-4 lg:gap-6 mb-5 lg:mb-12">
              <div className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.35em] text-brand">
            ...
            <p className="text-zinc-400 text-sm md:text-lg max-w-3xl leading-relaxed tracking-tight font-medium opacity-90 mx-auto lg:mx-0">The archive of high-stakes collisions. Every clash recorded here is a piece of Gzone history, where the UK's top-tier MCs settled scores and established dominance. Review the impact, track the views, and relive the battles that defined the "Most Wanted" division.</p>
          </div>
        ...
        <div className="bg-zinc-900/40 backdrop-blur-md rounded-[2.5rem] border border-white/10 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          <div className="sm:hidden">
            <div className="border-b border-white/10 bg-black/50 px-5 py-5">
              <p className="text-[9px] font-black uppercase tracking-[0.3em] text-brand">Season 1 Archive</p>
              <div className="mt-1 flex items-end justify-between gap-4">
                <h2 className="font-display text-2xl uppercase text-white">Battle Table</h2>
                <span className="text-[8px] font-black uppercase tracking-[0.2em] text-zinc-600">
                  {orderedBattles.length} clashes
                </span>
              </div>
            </div>
            ...
            <div className="divide-y divide-white/10">
              {orderedBattles.map((battle, index) => {
                ...
                const scheduleText = battle.ticketUrl ? "Postponed Date Coming Soon" : "Coming Soon";
                ...
                return (
                  <button
                    ...
                    className="block w-full px-4 py-5 text-left transition-colors active:bg-white/[0.04]"
                  >
                    <div className="flex items-center justify-between gap-3">
                      <span className="rounded-lg border border-brand/20 bg-brand/10 px-2.5 py-1.5 font-mono text-[10px] font-black text-brand">
                        {battle.episode || (!battle.isUnreleased ? `1x${String(index + 1).padStart(2, '0')}` : "TBC")}
                      </span>
                      <span className={`rounded-full border px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.18em] ${
                        battle.videoUrl
                    ...
                    <div className="my-5 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2">
                      <div className="flex min-w-0 items-center justify-end gap-1.5 text-right">
                        <span className="truncate font-display text-xl uppercase text-white">{mc1Name}</span>
                        {battle.winner === battle.mc1 && <Trophy size={14} className="shrink-0 text-brand" />}
                      </div>
                      <span className="px-1 text-[8px] font-black uppercase text-zinc-700">VS</span>
                      <div className="flex min-w-0 items-center gap-1.5">
                        {battle.winner === battle.mc2 && <Trophy size={14} className="shrink-0 text-brand" />}
                        <span className="truncate font-display text-xl uppercase text-white">{mc2Name}</span>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 overflow-hidden rounded-xl border border-white/5 bg-black/20">
                      <div className="px-3 py-3">
                        <span className="block text-[7px] font-black uppercase tracking-[0.2em] text-zinc-600">Date</span>
                        <span className="mt-1 flex items-center gap-1.5 text-[10px] font-bold uppercase text-zinc-300">
                          <Calendar size={11} className="text-brand" />
                          {battle.isUnreleased || shouldShowComingSoon ? scheduleText : (battle.date || "Scheduled")}
                        </span>
                      </div>
                      <div className="border-l border-white/5 px-3 py-3 text-right">
                        <span className="block text-[7px] font-black uppercase tracking-[0.2em] text-zinc-600">Views</span>
