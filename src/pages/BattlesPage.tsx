        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          ...
          <div className="flex gap-4 md:gap-8">
            <div className="bg-zinc-900/30 border border-white/5 p-8 rounded-3xl min-w-[180px]">
              ...
              <div className="text-[10px] text-zinc-500 uppercase tracking-widest">Last updated: Mar 21, 2026</div>
            </div>
            <div className="bg-zinc-900/30 border border-white/5 p-8 rounded-3xl min-w-[180px]">
              <div className="flex items-center gap-2 text-zinc-400 mb-2">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/5 bg-zinc-900/40">
                  <th className="px-6 py-5 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">Episode</th>
                  <th className="px-6 py-5 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">Battle</th>
                  <th className="px-6 py-5 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">Release Date</th>
                  <th className="px-6 py-5 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">Views</th>
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-6">
                        <Link to={`/battle/${battle.slug}`} className="font-mono text-brand text-sm hover:underline" aria-label={`Episode 1x${String(index + 1).padStart(2, '0')} details`}>1x{String(index + 1).padStart(2, '0')}</Link>
                      </td>
                      <td className="px-6 py-6">
                        <div className="block group-hover:translate-x-1 transition-transform">
                          <span className="font-display italic uppercase text-xl md:text-2xl text-zinc-100 group-hover:text-brand transition-colors flex items-center gap-3">
                            ...
                            <Link to={`/battle/${battle.slug}`} className="text-zinc-400 text-sm hover:text-brand transition-colors" aria-label={`Watch ${mc1?.name || battle.mc1} vs ${mc2?.name || battle.mc2}`}>VS</Link>
                            <span className="flex items-center gap-2">
