...
export default function VoteLivePage() {
  ...
  const castVote = async (battleId: string, choice: 'A' | 'B') => {
    ...
    if (locationState.status !== 'granted' || !locationState.result?.withinArena) {
      setVoteError('Arena location verification is required before your vote can be accepted.');
      return;
    }
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="mono text-[10px] font-black uppercase tracking-widest text-zinc-200">Arena geofence active</span>
            </div>
            <p className="mt-2 text-xs leading-relaxed text-zinc-300">
              Votes are only accepted inside {ARENA_LOCATION.address}.
            </p>
        ...
        {userVote ? (
          ...
          <div className="flex items-center gap-3 border border-orange-500/20 bg-orange-500/10 px-4 py-3 text-orange-200">
            ...
            <div className="text-xs font-bold leading-snug text-orange-100/80">
              Choose once. Your Google sign-in and arena GPS check keep the live count fair.
            </div>
          </div>
        )}
