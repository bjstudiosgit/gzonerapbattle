...
export default function StaffPage({ embedded = false }: StaffPageProps) {
  return (
    <div className={embedded ? "relative overflow-hidden bg-zinc-950 py-20 lg:py-28" : "min-h-screen pt-44 pb-24 relative overflow-hidden bg-zinc-950"}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <header className="mb-24 px-4 md:px-0 text-center lg:text-left">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-12">
            <div className="max-w-4xl">
              <h1 className="text-3xl sm:text-4xl md:text-6xl lg:text-[6.5rem] font-display uppercase leading-[0.9] tracking-tighter mb-12 shadow-2xl whitespace-nowrap">
                MEET THE <span className="text-brand drop-shadow-[0_0_30px_rgba(242,125,38,0.3)]">STAFF</span>
              </h1>
              
              <div className="h-1 w-48 bg-gradient-to-r from-brand to-transparent mb-12 origin-left mx-auto lg:mx-0" />
              
              <p className="text-zinc-400 text-sm md:text-lg max-w-4xl leading-relaxed tracking-tight font-medium opacity-80 border-l-0 lg:border-l-2 border-brand/20 lg:pl-6 py-2 mx-auto lg:mx-0">
                G Zone is raw, real, and uncensored. Built by people who understand pressure and performance, the league is led by figures who back their artists and stand on business when it matters. With Darren Stewart's discipline and Jay's refusal to play politics, every matchup carries weight.
              </p>
            </div>
        ...
        <div className="bg-zinc-950/40 backdrop-blur-3xl p-4 md:p-16 rounded-[5rem] border border-white/5 shadow-[0_0_150px_rgba(0,0,0,0.6)] ring-1 ring-white/10 overflow-hidden">
          <div className="mb-10 text-center md:text-left">
            ...
            <p className="text-brand font-bold tracking-[0.4em] text-[10px] md:text-xs">"Most Wanted"</p>
          </div>
          <HostsAndJudges embedded={embedded} excludeIds={embedded ? ["ginga-jay"] : []} />
        </div>
      </div>
    </div>
  );
