...
import { db, auth, OperationType, handleFirestoreError } from '../lib/voteFirebase';
...
export default function VotePage() {
  ...
  return (
    <div className="vote-system min-h-screen bg-zinc-950 crt-flicker relative selection:bg-orange-500 selection:text-white pt-32 md:pt-36">
      <div className="scanline" />
      
      {/* Header */}
      <header className="border-y border-zinc-800 bg-black/95 p-4 flex flex-col md:flex-row items-center justify-between gap-4 sticky top-28 md:top-32 z-50 backdrop-blur-xl">
...
function VoterView({ battles, votedBattles, user, authError, onLogin }: { battles: Battle[], votedBattles: Record<string, 'A' | 'B'>, user: any, authError: string | null, onLogin: () => void }) {
  ...
  const castVote = async (battleId: string, choice: 'A' | 'B') => {
    ...
    if (locationState.status !== 'granted' || !locationState.result?.withinArena) {
      setLocationState((current) => ({
        ...current,
        status: current.status === 'unsupported' ? 'unsupported' : 'blocked',
        error: current.error || votingCopy.location.required,
      }));
      return;
    }
    ...
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center p-12 text-center">
        <div className="bg-zinc-900 border border-zinc-800 p-12 shadow-2xl max-w-md">
          ...
          <h2 className="text-2xl font-black text-white mb-4 italic">{votingCopy.auth.requiredTitle}</h2>
          <p className="text-sm text-zinc-400 mb-8">{votingCopy.auth.requiredMessage}</p>
          {authError && (
  ...
  if (!activeBattle) {
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center p-12 text-center">
        ...
        <h2 className="text-2xl font-black uppercase opacity-70 italic">{votingCopy.status.standby.title}</h2>
        <p className="mono text-xs text-zinc-500 mt-2 uppercase tracking-widest">{votingCopy.status.standby.message}</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px_1fr] min-h-[calc(100vh-64px)] w-full">
        ...
        <button
          ...
          className={`w-full py-6 font-black text-xl md:text-2xl uppercase italic skew-btn-left transition-all z-10
            ${userVote === 'A' ? 'bg-orange-500 text-white shadow-[0_0_20px_#f97316]' : 
              locationLocked ? 'bg-zinc-800 text-zinc-500 cursor-not-allowed opacity-50' : 
              'bg-orange-600 hover:bg-orange-500 text-white cursor-pointer'}`}
        >
          {userVote === 'A' ? withArtistName(votingCopy.voting.lockedFor, activeBattle.artistA) : locationLocked ? votingCopy.location.requiredTitle : withArtistName(votingCopy.voting.voteButton, activeBattle.artistA)}
        </button>
      </div>
      ...
      <div className="bg-zinc-950/50 border-x border-zinc-800 flex flex-col p-6 text-center space-y-12">
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-center gap-2 mt-4">
            ...
            <span className="mono text-[10px] uppercase text-orange-500 font-black tracking-widest">{votingCopy.status.open.title}</span>
          </div>
        </div>
        ...
        <div className="space-y-6 flex-1 flex flex-col justify-center">
            <div className="p-4 bg-black/40 rounded border border-zinc-800/50 backdrop-blur-sm">
                <div className="flex justify-between mono text-[9px] mb-2 font-black tracking-widest">
                    <span className="text-zinc-500">{votingCopy.status.open.title}</span>
                    <span className="text-green-500">{votingCopy.voting.confirmed}</span>
                </div>
                <div className="text-[10px] text-zinc-400 leading-snug italic uppercase text-left opacity-70">
                    {votingCopy.voting.instruction} {votingCopy.voting.helper}
                </div>
            </div>

            <div className={`p-4 rounded border backdrop-blur-sm text-left ${
              isLocationGranted
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="mono text-[10px] uppercase tracking-widest font-black text-zinc-300">
                  ...
                  {locationState.result && (
                    <p className="mt-2 text-[10px] text-zinc-500 uppercase leading-relaxed">
                      Distance {formatMeters(locationState.result.distanceMeters)} • Accuracy {formatMeters(locationState.result.accuracy)}
                    </p>
                  )}
                  {locationState.error && (
                    <p className="mt-2 text-[10px] text-red-300 uppercase leading-relaxed">
                      {locationState.error}
                    </p>
                  )}
                </div>
                <button
                  type="button"
                  onClick={requestArenaCheck}
                  disabled={locationState.status === 'checking'}
                  className="shrink-0 border border-white/10 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-white hover:border-orange-500/40 disabled:opacity-50"
                >
            ...
            {userVote && (
                ...
                >
                    ...
                    <p className="text-sm font-black uppercase tracking-tighter">{votingCopy.voting.confirmed}</p>
                    <p className="text-[10px] mono text-zinc-500 mt-1 uppercase">
                      {withArtistName(
      ...
      <div className={`voter-card flex flex-col p-8 justify-between relative group text-right ${userVote === 'B' ? 'active-glow-white bg-white/5' : userVote ? 'opacity-30' : ''}`}>
        ...
        >
          {userVote === 'B' ? withArtistName(votingCopy.voting.lockedFor, activeBattle.artistB) : locationLocked ? votingCopy.location.requiredTitle : withArtistName(votingCopy.voting.voteButton, activeBattle.artistB)}
        </button>
      </div>
    </div>
  );
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t border-zinc-800/50">
              ...
              {battle.status === 'active' && (
                <button 
                  ...
                  className="bg-zinc-100 text-black px-4 py-1.5 text-[10px] font-black uppercase flex items-center gap-2 hover:bg-white transition-colors"
                >
  ...
  if (!activeBattle) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center border border-zinc-800 bg-zinc-950/20">
        ...
        <h2 className="text-5xl font-black uppercase opacity-20 tracking-tighter italic select-none">{votingCopy.status.noBattle.title}</h2>
      </div>
    );
  }
    ...
    <div className="min-h-[70vh] grid grid-cols-1 lg:grid-cols-2 gap-12 items-center animate-in fade-in duration-1000">
      ...
      <div className="lg:col-span-2 text-center space-y-4 mb-8">
        <AnimatePresence mode="wait">
            {activeBattle.status === 'active' ? (
                ...
                >
                    {votingCopy.status.closed.title}
                </motion.div>
            )}
        </AnimatePresence>
      ...
      <div className="lg:col-span-2 flex justify-center pt-12">
        <div className="bg-zinc-900/50 border border-zinc-800 px-12 py-4 flex items-center gap-6 rounded-full backdrop-blur-md">
            ...
            <span className="mono text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500">{votingCopy.voting.total}</span>
        </div>
      </div>
    </div>
  );
