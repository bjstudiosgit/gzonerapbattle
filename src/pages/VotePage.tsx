...
function VoterView({ battles, votedBattles, user, authError, onLogin }: { battles: Battle[], votedBattles: Record<string, 'A' | 'B'>, user: any, authError: string | null, onLogin: () => void }) {
  ...
  const requestArenaCheck = () => {
    if (!navigator.geolocation) {
      setLocationState({
        status: 'unsupported',
        error: 'This device does not support GPS location, so arena voting cannot be verified.',
        result: null,
      ...
      (position) => {
        ...
        setLocationState({
          ...
          error: result.withinArena
            ? null
            : 'Voting is restricted to the arena floor at Peacock Gymnasium.',
          result,
          lastCheckedAt: Date.now(),
        });
      },
      (error) => {
        let message = 'Location access is required to vote inside the arena.';
        if (error.code === error.PERMISSION_DENIED) {
          message = 'Location access was denied. Enable GPS permission and try again.';
        } else if (error.code === error.POSITION_UNAVAILABLE) {
          message = 'Your location could not be determined. Move to a clearer signal area and try again.';
  ...
  const castVote = async (battleId: string, choice: 'A' | 'B') => {
    ...
    if (locationState.status !== 'granted' || !locationState.result?.withinArena) {
      setLocationState((current) => ({
        ...
        error: current.error || 'Arena location verification is required before voting.',
      }));
      return;
    }
    ...
    try {
      await setDoc(doc(db, 'votes', voteId), {
        ...
        location: {
          ...
          accuracy: locationState.result.accuracy,
          distanceMeters: locationState.result.distanceMeters,
          venue: ARENA_LOCATION.address,
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px_1fr] min-h-[calc(100vh-64px)] w-full">
      ...
      <div className={`voter-card flex flex-col p-8 justify-between relative group ${userVote === 'A' ? 'active-glow-orange bg-orange-950/20' : userVote ? 'opacity-30' : ''}`}>
        ...
        >
          {userVote === 'A' ? 'LOCKED FOR A' : locationLocked ? 'ARENA CHECK REQUIRED' : `Vote for ${activeBattle.artistA}`}
        </button>
      </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="mono text-[10px] uppercase tracking-widest font-black text-zinc-300">Arena geofence</span>
                  </div>
                  <p className="text-[10px] text-zinc-300 uppercase leading-relaxed">
                    Voting is only enabled inside {ARENA_LOCATION.address}.
                  </p>
      ...
      <div className={`voter-card flex flex-col p-8 justify-between relative group text-right ${userVote === 'B' ? 'active-glow-white bg-white/5' : userVote ? 'opacity-30' : ''}`}>
        ...
        >
          {userVote === 'B' ? 'LOCKED FOR B' : locationLocked ? 'ARENA CHECK REQUIRED' : `Vote for ${activeBattle.artistB}`}
        </button>
      </div>
    </div>
  );
