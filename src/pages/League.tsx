  ...
  return (
    <div className="min-h-screen pt-44 pb-24 relative overflow-hidden bg-zinc-950">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-3 gap-10 mb-32 relative px-4 md:px-0">
          {top3.map((mc, index) => {
                ...
                <div className={`absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity ${tier.text}`}>
                 ...
                 <div className="flex items-center justify-between mb-12">
                  ...
                  <div className="text-right">
                    <p className={`text-[10px] font-black tracking-widest opacity-60 mb-1 ${tier.text}`}>Target Priority</p>
                    <p className={`text-4xl font-display tracking-tighter drop-shadow-md ${tier.text}`}>{mc.points}</p>
                  </div>
                </div>

                <div className="text-center relative">
                  {/* WANTED Badge for League Podium */}
                  ...
                  <div className="relative inline-block mb-10">
                    <div className={`w-28 h-28 md:w-36 md:h-36 rounded-[2.5rem] overflow-hidden border-2 transition-all duration-500 bg-zinc-900 ${
                      tier.border
                    }`}>
                      {mc.image ? (
                        <img 
                  ...
                  <Link to={`/mc/${mc.slug}`} className="block group/link">
                    <div className="flex items-center justify-center gap-1 mb-4">
                      {[...Array(index === 0 ? 5 : index === 1 ? 4 : 3)].map((_, i) => (
                        <Star key={i} size={14} className="text-yellow-500 fill-yellow-500 animate-pulse" style={{ animationDelay: `${i * 150}ms` }} />
                      ))}
                    </div>
